...
def stencil_static3(
    ...
    *,
    ...
    with computation(PARALLEL), interval(...):
        ...
        tem1 = 1.0 - 2.0 * (tkemx - tkemean) / dtke
    ...
    with computation(FORWARD), interval(1, -1):
        ...
        if cnvflg:
            if k_idx > kb and k_idx < kbcon1:
                ...
                rfact = 1.0 + delta * cp * gamma * to / hvap
                cina = cina + dz1 * (g / (cp * to)) * dbyo / (1.0 + gamma) * rfact
                # val   = 0.
                cina = (
                    (
                        cina
    ...
    with computation(FORWARD), interval(1, -1):
        ...
        if cnvflg:
            if k_idx > kb and k_idx < ktcon:
                ...
                qrch = qeso + gamma * dbyo / (hvap * (1.0 + gamma))
                # j
                ...
                if k_idx >= kbcon:
                    rfact = 1.0 + delta * cp * gamma * to / hvap
                    buo = buo + (g / (cp * to)) * dbyo / (1.0 + gamma) * rfact

                    # val = 0.
                    buo = (
                        (buo + g * delta * (qeso - qo)) if ((qeso - qo) > 0.0) else buo
                    )
    ...
    with computation(FORWARD), interval(1, -1):
        ...
        if flg:
            if k_idx >= ktcon and k_idx < kbm:
                ...
                gamma = el2orc * qeso / (to ** 2)
                rfact = 1.0 + delta * cp * gamma * to / hvap
                aa1 = aa1 + dz1 * (g / (cp * to)) * dbyo / (1.0 + gamma) * rfact
