...
from config import DTYPE_FLOAT, BACKEND
import gt4py as gt
    ...
    *,
    ...
    with computation(FORWARD), interval(1, -1):
        if cnvflg and (index_k > kb) and (index_k < ktcon):
            ...
            ecko2 = (
                (1.0 - tem1) * ecko2[0, 0, -1]
                + 0.5 * tem * (ctro2[0, 0, 0] + ctro2[0, 0, -1])
            ) / factor
...
def samfshalcnv_aerosols(
    ...
    index_k = gt.storage.from_array(
        index_ijk_np[2] + 1, BACKEND, default_origin, shape_2d, dtype=int
    )  # index STARTING FROM 1
    dtime_max_arr = gt.storage.empty(
        BACKEND, default_origin, shape_2d, dtype=DTYPE_FLOAT
    )

    # Begin
    ## Check if aerosols are present
    if (ntc <= 0) or (itc <= 0) or (ntr <= 0):
        return
    if ntr < itc + ntc - 3:
        return
    ...
    for n in range(ntc):
        ...
        set_work_arrays(
            qtr, xmb, delp, kmax, kb, cnvflg, index_k, qaero, xmbp, ctro2, ecko2
        )
