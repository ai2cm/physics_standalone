...
def run(data_dict, timings):
    ...
    ### K-indices field ###
    k_idx = gt.storage.from_array(
        np.indices(shape)[2] + 1, BACKEND, default_origin, dtype=DTYPE_INT
    )
    ...
    if exit_routine(cnvflg, im):
        return

    # Initialize further parameters and arrays
    init_par_and_arr(
        islimsk, c0, t1, c0t, cnvw, cnvc, ud_mf, dt_mf, c0s=c0s, asolfac=asolfac, d0=d0
    )
    ...
    stencil_static2(
        cnvflg, pdot, dot_kbcon, islimsk, k_idx, kbcon, kb, pfld_kb, pfld_kbcon
    )
    ...
    if ntk > 0:
        qtr_ntk = gt.storage.from_array(
            qtr[np.newaxis, :, :, ntk - 1], BACKEND, default_origin
        )
        stencil_static3(
            sumx, tkemean, cnvflg, k_idx, kb, kbcon, zo, qtr_ntk, clamt, clam=clam
        )
    #    qtr[:,:,ntr] = qtr_ntr[0,:,:]
    # else:
    ...
    stencil_static10(
        cina, cnvflg, k_idx, kb, kbcon1, zo, qeso, to, dbyo, qo, pdot, islimsk
    )
    ...
    if ntk > 0:
        ...
        tke_contribution(
            cnvflg, k_idx, kb, ktop, eta, xmb, pfld, t1, sigmagfm, qtr_ntk, betaw=betaw
        )
