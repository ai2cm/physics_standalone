...
import random
import gt4py as gt
from shalconv.config import BACKEND, DTYPE_FLOAT, DTYPE_INT, SERIALBOX_DIR
...
def numpy_dict_to_gt4py_dict(data_dict, backend=BACKEND):
    ...
    for var in data_dict:
        ...
        if (ndim > 0) and (ndim <= 2) and (data.size >= 2):
            ...
            new_data_dict[var] = gt.storage.from_array(
                arrdata, backend, default_origin, dtype=dtype
            )

        elif (
            ndim == 3
        ):  # 3D array qntr(horizontal dimension, vertical dimension, number of tracers)
...
def read_random_input(length, ix, ntile, ncount, path):
    """
    ...
    for key in data_list[0]:
        ...
        if key == "fscav":
            ...
            output[key] = np.zeros(
                (length, data.shape[1], data.shape[2]), dtype=data.dtype, order="F"
            )
