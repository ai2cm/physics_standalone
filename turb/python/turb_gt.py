...
def satmedmfvdif_gt(im, ix, km, ntrac, ntcw, ntiw, ntke,
    ...
    # 3D GT storage
    qcko   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,km+1,ntrac), default_origin=(0,0,0))
    qcdo   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,km+1,ntrac), default_origin=(0,0,0))
    f2     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km*(ntrac-1)), default_origin=(0,0,0))
    pcnvflg_v2 = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,km+1,ntrac), default_origin=(0,0,0))
    scuflg_v2  = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,km+1,ntrac), default_origin=(0,0,0))
    q1_gt      = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,km+1,ntrac), default_origin=(0,0,0))
    ...
    zi    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    zl    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    zm    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ckz   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    chz   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    tke   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    rdzt  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    prn   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xkzo  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xkzmo = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    pix   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    theta = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qlx   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    slx   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thvx  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qtx   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thlx  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thlvx = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thlvx_0 = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    svx   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thetae = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    gotvx = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    plyr  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    cfly  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    bf    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    dku   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    dkt   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    dkq   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    radx  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    shr2  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    tcko  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    tcdo  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ucko  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ucdo  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    vcko  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    vcdo  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcko_0    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcko_ntke = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcdo_0    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcdo_ntke = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    buou   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xmf    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xlamue = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    rhly   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qstl   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    buod   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xmfd   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xlamde = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    rlam   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ele    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    elm    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    prod   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    rle    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    diss   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ad     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ad_p1  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    f1     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    f1_p1  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    al     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    au     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))    
    f2_km  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    f2_p1  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))

    # 1D GT storages extended into 3D
    gdx     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xkzm_hx = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xkzm_mx = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    kx1     = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    z0      = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    kpblx   = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    hpblx   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    pblflg  = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,1,km+1), default_origin=(0,0,0))
    sfcflg  = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,1,km+1), default_origin=(0,0,0))
    pcnvflg = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,1,km+1), default_origin=(0,0,0))
    scuflg  = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,1,km+1), default_origin=(0,0,0))
    radmin  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    mrad    = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    krad    = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    lcld    = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    kcld    = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    flg     = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,1,km+1), default_origin=(0,0,0))
    rbup    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    rbdn    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    sflux   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thermal = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    crb     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    dtdz1   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ustar   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    zol     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    phim    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    phih    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    wscale  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    vpert   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    radj    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    zl_0    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))

    # Mask/Index Array
    mask = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    ...
    for k in range(km):
        for i in range(im):
            rtg[i,k,ntke-1] = rtg[i,k,ntke-1] + (f1[i,0,k] - q1[i,k,ntke-1]) * rdt
    ...
    if ntrac1 >= 2:
        for kk in range(1,ntrac1):
            ...
            for k in range(km):
                for i in range(im):
                    rtg[i,k,kk] = rtg[i,k,kk] + ((f2[i,0,k+is_] - q1[i,k,kk])*rdt)
...
def storage_to_numpy_and_assert_equal(gt_storage, numpy_array):
    ...
    if numpy_array.ndim == 1:
        ...
        temp = gt_storage
        temp = temp[0:numpy_array.shape[0], 0:numpy_array.shape[1],0:numpy_array.shape[2]]
        temp2 = np.zeros(temp.shape)
        temp2[:,:,:] = temp
        #np.testing.assert_allclose(temp2,numpy_array,rtol=1e-13,atol=0)
        np.testing.assert_array_equal(temp2,numpy_array)
...
def init(zi      : FIELD_FLT,
    ...
    with computation(PARALLEL), interval(0,-1):
        ...
        if mask[0,0,0] < kinver[0,0,0]:
            ...
            xkzo = xkzm_hx[0,0,0] * min(1.0,exp(-((1.0 - ptem) * (1.0 - ptem) * 10.0)))

            if ptem >= xkzm_s:
                xkzmo = xkzm_mx[0,0,0]
                kx1 = mask[0,0,0] + 1
            else:
                tem1 = min(1.0, exp(-((1.0 - prsi[0,0,1] * tx2[0,0,0]) * (1.0 - prsi[0,0,1] * tx2[0,0,0]) * 5.0)))
    ...
    with computation(PARALLEL), interval(0,-2):
            ...
            bf    = chx * ((slx[0,0,1] - slx[0,0,0]) * rdzt[0,0,0]) + cqx * ((qtx[0,0,1] - qtx[0,0,0]) * rdzt[0,0,0])
    ...
    with computation(PARALLEL):
        with interval(0,1):
            ...
            if pblflg[0,0,0]:
                ...
                tem1 = 1e-7 * (max(sqrt(u10m[0,0,0]**2 + v10m[0,0,0]**2),1.0) / (f0 * z0[0,0,0]))
...
def part3a(crb     : FIELD_FLT,
    ...
    with computation(FORWARD):
        with interval(0,1):
            ...
            if flg[0,0,0] == 0:
                rbdn = rbup[0,0,0]
                rbup = (thlvx[0,0,0] - thermal[0,0,0]) * (g*zl[0,0,0]/thlvx_0[0,0,0])/max(u1[0,0,0]**2 + v1[0,0,0]**2,1.0)
                kpblx = mask[0,0,0]
...
def part3a1(crb     : FIELD_FLT,
    ...
    with computation(PARALLEL), interval(0,1):
        ...
        if pblflg[0,0,0]:
            wscale = max((ust3 + wfac * vk * wst3 * sfcfrac)**h1,ustar[0,0,0]/aphi5)
...
def part3e(flg    : FIELD_BOOL,
    ...
    with computation(BACKWARD):
        with interval(-1,None):
            if flg[0,0,0] and (mask[0,0,0] <= lcld[0,0,0]) and (qlx[0,0,0] >= qlcr):
                kcld = mask[0,0,0]
                flg  = 0
...
@gtscript.stencil(backend=backend)
def part4a(pcnvflg_v2      : FIELD_BOOL,
           q1              : FIELD_FLT,
           qcdo            : FIELD_FLT,
           qcko            : FIELD_FLT,
...
def part6(bf      : FIELD_FLT,
    ...
    with computation(PARALLEL), interval(0,-1):
            tem = 0.5 * (elm[0,0,0] + elm[0,0,1]) * sqrt(0.5 * (tke[0,0,0] + tke[0,0,1]))
    ...
    with computation(PARALLEL):
        with interval(0,1):
            ...
            buop = 0.5 * (gotvx[0,0,0] * sflux[0,0,0] + (-dkt[0,0,0] * bf[0,0,0] + ptem))

            if scuflg[0,0,0] and mrad[0,0,0] == 0:
                ptem1 = 0.5 * (u1[0,0,1] - u1[0,0,0]) * rdzt[0,0,0] * xmfd[0,0,0] * (ucdo[0,0,0] + ucdo[0,0,1] - u1[0,0,0] - u1[0,0,1])
            else:
                ptem1 = 0.0

            if scuflg[0,0,0] and mrad[0,0,0] == 0:
                ptem2 = 0.5 * (v1[0,0,1] - v1[0,0,0]) * rdzt[0,0,0] * xmfd[0,0,0] * (vcdo[0,0,0] + vcdo[0,0,1] - v1[0,0,0] - v1[0,0,1])
            else:
                ptem2 = 0.0
        
            shrp = 0.5 * (dku[0,0,0] * shr2[0,0,0] + ptem1 + ptem2 + (stress[0,0,0] * ustar[0,0,0] * phim[0,0,0] / (vk*zl[0,0,0])))
        ...
        with interval(1,-1):
            ...
            buop = 0.5 * ((-dkt[0,0,-1] * bf[0,0,-1]) + (-dkt[0,0,0] * bf[0,0,0])) + ptem1 + ptem2

            tem1 = (u1[0,0,1]-u1[0,0,0]) * rdzt[0,0,0]
            tem2 = (u1[0,0,0]-u1[0,0,-1]) * rdzt[0,0,-1]

            if pcnvflg[0,0,0] and mask[0,0,0] <= kpbl[0,0,0]:
                ptem1 = 0.5 * (xmf[0,0,0] * tem1 + xmf[0,0,-1] * tem2) * (u1[0,0,0] - ucko[0,0,0])
            else:
                ptem1 = 0.0
            
            if scuflg[0,0,0]:
                if mask[0,0,0] >= mrad[0,0,0] and mask[0,0,0] < krad[0,0,0]:
                    ptem2 = 0.5 * (xmfd[0,0,0] * tem1 + xmfd[0,0,-1] * tem2) * (ucdo[0,0,0] - u1[0,0,0])
                else:
                    ptem2 = 0.0
            else:
                ptem2 = 0.0
            
            shrp = 0.5 * ((dku[0,0,-1] * shr2[0,0,-1]) + (dku[0,0,0] * shr2[0,0,0])) + ptem1 + ptem2
            tem1 = (v1[0,0,1] - v1[0,0,0]) * rdzt[0,0,0]
            tem2 = (v1[0,0,0] - v1[0,0,-1]) * rdzt[0,0,-1]

            if pcnvflg[0,0,0] and mask[0,0,0] <= kpbl[0,0,0]:
                ptem1 = 0.5 * (xmf[0,0,0] * tem1 + xmf[0,0,-1] * tem2) * (v1[0,0,0] - vcko[0,0,0])
            else:
                ptem1 = 0.0
            
            if scuflg[0,0,0]:
                if mask[0,0,0] >= mrad[0,0,0] and mask[0,0,0] < krad[0,0,0]:
                    ptem2 = 0.5 * (xmfd[0,0,0] * tem1 + xmfd[0,0,-1] * tem2) * (vcdo[0,0,0] - v1[0,0,0])
                else:
                    ptem2 = 0.0
            else:
                ptem2 = 0.0
...
@gtscript.stencil(backend=backend)
def part8(diss : FIELD_FLT,
          prod : FIELD_FLT,
          rle  : FIELD_FLT,
          tke  : FIELD_FLT,
          dtn  : float):
    with computation(PARALLEL), interval(...):
        diss = max(min(rle[0,0,0] * tke[0,0,0] * sqrt(tke[0,0,0]), prod[0,0,0] + tke[0,0,0] / dtn), 0.0)
        tke  = max(tke[0,0,0] + dtn * (prod[0,0,0] - diss[0,0,0]),tkmin)

# Possible stencil name : tke_up_down_prop_1
@gtscript.stencil(backend=backend)
def part9(pcnvflg   : FIELD_BOOL,
          qcdo_ntke : FIELD_FLT,
          qcko_ntke : FIELD_FLT,
          scuflg    : FIELD_BOOL,
          tke       : FIELD_FLT):
...
def part10(kpbl      : FIELD_INT,
    ...
    with computation(FORWARD), interval(1,None):
        ...
        if pcnvflg[0,0,0] and mask[0,0,0] <= kpbl[0,0,0]:
            qcko_ntke = ((1.0-tem)*qcko_ntke[0,0,-1] + tem*(tke[0,0,0]+tke[0,0,-1]))/(1.0 + tem)

# Possible stencil name : tke_up_down_prop_3
@gtscript.stencil(backend=backend)
def part11(ad        : FIELD_FLT,
           f1        : FIELD_FLT,
           krad      : FIELD_INT,
           mask      : FIELD_INT,
           mrad      : FIELD_INT,
    ...
    with computation(BACKWARD), interval(...):
        ...
        if scuflg[0,0,0] and mask[0,0,0] < krad[0,0,0] and mask[0,0,0] >= mrad[0,0,0]:
            qcdo_ntke = ((1.0-tem)*qcdo_ntke[0,0,1] + tem*(tke[0,0,0]+tke[0,0,1]))/(1.0 + tem)
...
def part12(ad        : FIELD_FLT,
    ...
    with computation(FORWARD):
        with interval(0,1):
            ...
            if pcnvflg[0,0,0] and mask[0,0,0] < kpbl[0,0,0]:
                tem = qcko_ntke[0,0,0] + qcko_ntke[0,0,1] - (tke[0,0,0] + tke[0,0,1])
            ...
            if scuflg[0,0,0] and mask[0,0,0] >= mrad[0,0,0] and mask[0,0,0] < krad[0,0,0]:
                tem = qcdo_ntke[0,0,0] + qcdo_ntke[0,0,1] - (tke[0,0,0] + tke[0,0,1])
...
def mfpblt(im,ix,km,kmpbl,ntcw,ntrac1,delt, 
    ...
    wu2     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qtu     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qtx     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xlamuem = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thlu    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qtu     = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thlu    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    kpblx   = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    kpbly   = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    rbup    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    rbdn    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    flg     = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,1,km+1), default_origin=(0,0,0))
    hpblx   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xlamavg = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    sumx    = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    sigma   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    scaldfunc = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))

    qcko_1 = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcko_ntcw = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcko_track = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
...
def mfpblt_s1(buo     : FIELD_FLT,
    ...
    with computation(PARALLEL), interval(...):
        if cnvflg[0,0,0]:
            ...
            if mask[0,0,0] < kpbl[0,0,0]:
                xlamue = ce0 * (1.0/(zm[0,0,0] + dz) + 1.0/max(hpbl[0,0,0]-zm[0,0,0]+dz, dz))
            else:
                xlamue = ce0 / dz
        ...
        with interval(1,None):
            if cnvflg[0,0,0]:
                ...
                if dq > 0.0:
                    ...
                    qlu = dq / (1.0 + gamma)
                    qtu = qs + qlu
                    thvu = (thlu[0,0,0] + pix[0,0,0] * elocp * qlu) * (1.0 + fv * qs - qlu)
                else:
    ...
    with computation(FORWARD):
        with interval(0,1):
            if cnvflg[0,0,0]:
                wu2 = (4.0 * buo[0,0,0] * zm[0,0,0]) / (1.0 + (0.5 * 2.0 * xlamue[0,0,0] * zm[0,0,0]))
        with interval(1,None):            
            if cnvflg[0,0,0]:
                dz = zm[0,0,0] - zm[0,0,-1]
                tem = 0.25 * 2.0 * (xlamue[0,0,0] + xlamue[0,0,-1]) * dz
                wu2 = (((1.0 - tem) * wu2[0,0,-1]) + (4.0 * buo[0,0,0] * dz)) / (1.0 + tem)
...
@gtscript.stencil(backend=backend)
def mfpblt_s1a(cnvflg      : FIELD_BOOL,
               hpblx       : FIELD_FLT,
               kpblx       : FIELD_INT,
               mask        : FIELD_INT,
               rbdn        : FIELD_FLT,
...
@gtscript.stencil(backend=backend)
def mfpblt_s2(cnvflg     : FIELD_BOOL,
              gdx        : FIELD_FLT,
              hpbl       : FIELD_FLT,
              hpblx      : FIELD_FLT,
              kpbl       : FIELD_INT,
    ...
    with computation(FORWARD):
        with interval(0,1):
            if cnvflg[0,0,0]:
                ...
                sigma = min(max((3.14 * tem * tem) / (gdx[0,0,0] * gdx[0,0,0]), 0.001), 0.999)

                if sigma[0,0,0] > a1:
                    scaldfunc = max(min((1.0 - sigma[0,0,0]) * (1.0 - sigma[0,0,0]), 1.0), 0.0)
                else:
                    scaldfunc = 1.0
        with interval(1,None):
    ...
    with computation(FORWARD): 
        ...
        with interval(1,None):
            ...
            if cnvflg[0,0,0] and (mask[0,0,0] <= kpbl[0,0,0]):
                ucko = ((1.0-tem) * ucko[0,0,-1] + (tem + pgcon)*u1[0,0,0] + (tem - pgcon)*u1[0,0,-1])/factor
                vcko = ((1.0-tem) * vcko[0,0,-1] + (tem + pgcon)*v1[0,0,0] + (tem - pgcon)*v1[0,0,-1])/factor
...
def mfscu(im,ix,km,kmscu,ntcw,ntrac1,delt,
    ...
    wd2 = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qtx = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    hrad = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    krad1 = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    thld  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qtd   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    thlvd = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ra1   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    ra2   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    radj  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    flg   = gt_storage.zeros(backend=backend, dtype=DTYPE_BOOL, shape=(im,1,km+1), default_origin=(0,0,0))
    xlamdem = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    mradx = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    mrady = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
    sumx  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xlamavg = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    xmfd   = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    sigma  = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    scaldfunc = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcdo_1 = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcdo_ntcw = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(im,1,km+1), default_origin=(0,0,0))
    qcdo_track = gt_storage.zeros(backend=backend, dtype=DTYPE_INT, shape=(im,1,km+1), default_origin=(0,0,0))
        ...
        for i in range(im):
            if cnvflg[i,0,0]:
                ...
                if (k >= mrad[i,0,0]) and (k < krad[i,0,0]):
                    ...
                    xlamde[i,0,k] = ce0 * (ptem+1.0/max(hrad[i,0,0] - zm[i,0,k] + dz, dz))
                else:
                    xlamde[i,0,k] = ce0 / dz
    ...
    with computation(FORWARD), interval(...):
        ...
        if krad[0,0,0] == mask[0,0,0]:
            if cnvflg[0,0,0]:
                hrad = zm[0,0,0]
                krad1 = mask[0,0,0] - 1
                tem1 = max(cldtime * radmin[0,0,0]/(zm[0,0,1] - zm[0,0,0]), -3.0)
...
@gtscript.stencil(backend=backend)
def mfscu_s0b(cnvflg  : FIELD_BOOL,
              flg     : FIELD_BOOL,
              krad    : FIELD_INT,
              mask    : FIELD_INT,
              mrad    : FIELD_INT,
    ...
    with computation(FORWARD), interval(...):
        if mask[0,0,0] == krad1[0,0,0]:
            if cnvflg[0,0,0]:
                ...
                wd2 = (bb2 * buo[0,0,1] * dz) / (1.0 + (0.5 * bb1 * xlamde[0,0,0] * dz))

@gtscript.stencil(backend=backend)
def mfscu_s2(buo       : FIELD_FLT,
             cnvflg    : FIELD_BOOL,
             flg       : FIELD_BOOL,
             krad      : FIELD_INT,
             krad1     : FIELD_INT,
...
@gtscript.stencil(backend=backend)
def mfscu_s3(cnvflg    : FIELD_BOOL,
             gdx       : FIELD_FLT,
             krad      : FIELD_INT,
             mask      : FIELD_INT,
             mrad      : FIELD_INT,
    ...
    with computation(FORWARD):
        with interval(0,1):
            if cnvflg[0,0,0]:
                tem1 = (3.14 * (0.2 / xlamavg[0,0,0]) * (0.2 / xlamavg[0,0,0])) / (gdx[0,0,0] * gdx[0,0,0])
...
@gtscript.stencil(backend=backend)
def mfscu_s3a(cnvflg    : FIELD_BOOL,
              krad      : FIELD_INT,
              mask      : FIELD_INT,
              thld      : FIELD_FLT,
              thlx      : FIELD_FLT):
...
@gtscript.stencil(backend=backend)
def mfscu_s4(cnvflg    : FIELD_BOOL,
             krad      : FIELD_INT,
             mask      : FIELD_INT,
             mrad      : FIELD_INT,
             pix       : FIELD_FLT,
...
def tridin(l,n,nt,cl,cm,cu,r1,r2,au,a1,a2):
    fk = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(l,1,n+1), default_origin=(0,0,0))
    fkk = gt_storage.zeros(backend=backend, dtype=DTYPE_FLT, shape=(l,1,n+1), default_origin=(0,0,0))
    ...
    for k in range(1,n-1):
        for i in range(l):
            ...
            a1[i,0,k]  = fkk[i,0,k] * (r1[i,0,k] - cl[i,0,k-1] * a1[i,0,k-1])

    for kk in range(nt):
        is_ = kk * n
        for k in range(1,n-1):
            for i in range(l):
                a2[i,0,k+is_] = fkk[i,0,k] * (r2[i,0,k+is_] - cl[i,0,k-1] * a2[i,0,k+is_-1])

    for i in range(l):
        fk[i,0,0] = 1 / (cm[i,0,n-1] - cl[i,0,n-2] * au[i,0,n-2])
        a1[i,0,n-1] = fk[i,0,0] * (r1[i,0,n-1] - cl[i,0,n-2] * a1[i,0,n-2])

    for k in range(nt):
        is_ = k * n
        for i in range(l):
            a2[i,0,n+is_-1] = fk[i,0,0] * (r2[i,0,n+is_-1] - cl[i,0,n-2]*a2[i,0,n+is_-2])
    ...
    for kk in range(nt):
        ...
        for k in range(n-2,-1,-1):
            for i in range(l):
                a2[i,0,k+is_] = a2[i,0,k+is_] - au[i,0,k]*a2[i,0,k+is_+1]

    return au, a1, a2

@gtscript.stencil(backend=backend)
def tridi2(a1      : FIELD_FLT,
           a2      : FIELD_FLT,
           au      : FIELD_FLT,
           cl      : FIELD_FLT,
           cm      : FIELD_FLT,
