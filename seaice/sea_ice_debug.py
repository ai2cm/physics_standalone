...
#  --- ...  update sea ice temperature
    ...
    stsice = ser.read("stsice", sp)  # stsice fix
...
#         sfcnsw is the net shortwave flux (direction: dn-up)
    ...
    if any(fice[flag] < cimin):
        ...
        fice[flag & (fice < cimin)] = cimin
        tice[flag & (fice < cimin)]= tgice
        tskin[flag & (fice < cimin)]= tgice
...
# TODO: write more efficiently, save mask as new variable?
    if any(snowd[flag] > (2.*hice[flag])):
        print('warning: too much snow :',
              snowd[flag & (snowd > (2.*hice))])
        snowd[flag & (snowd > (2.*hice))] = \
                hice[flag & (snowd > (2.*hice))] + \
                hice[flag & (snowd > (2.*hice))]
        print('fix: decrease snow depth to:',
              snowd[flag & (snowd  > (2.*hice))])

    # check qs1     
    ser_2 = ser.read("qs1", sp)
    print('fortran\n', ser_2[~np.isclose(qs1,ser_2,equal_nan = True)],\
 '\n python \n', qs1[~np.isclose(qs1,ser_2, equal_nan =True)])
