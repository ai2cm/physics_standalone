...
def run(in_dict, backend=BACKEND):
    ...
    # initialize local storages for split of stencil
    storage_shape = (in_dict['flag_iter'].shape[0],1,1)
    hfi   = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0)) 
    hfd   = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    sneti = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    focn  = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    snof  = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    ffw   = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    theta1= gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    rch   = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    evapi = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    evapw = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
    rho   = gt.storage.zeros(backend=backend, dtype=DT_F, shape=storage_shape, mask=None, default_origin=(0, 0, 0))
...
# new variables:
        ...
        rho=rho)
    ice3lay(
        islimsk=in_dict['islimsk'],
        flag_iter=in_dict['flag_iter'],
        hice=out_dict['hice'],
        fice=out_dict['fice'],
        tice=out_dict['tice'],
        snowd=out_dict['weasd'],
        stc0=out_dict['stc0'],
        stc1=out_dict['stc1'],
        gflux=out_dict['gflux'],
        snowmt=out_dict['snowmt'],
        snwdph=out_dict['snwdph'],
        hfi=hfi,
        hfd=hfd,
        sneti=sneti,
        focn=focn,
        snof=snof,
        delt=scalar_dict['delt'])
    sfc_sice_p2(
        islimsk=in_dict['islimsk'],
        flag_iter=in_dict['flag_iter'],
        fice=out_dict['fice'],
        ...
        stc1=out_dict['stc1'],
        q1=in_dict['q1'],
        tskin=out_dict['tskin'],
        qsurf=out_dict['qsurf'],
        evap=out_dict['evap'],
        hflx=out_dict['hflx'],
# new variables:
        ffw=ffw,
