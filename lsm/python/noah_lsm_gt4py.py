...
def fpvs_fn(c1xpvs, c2xpvs, tbpvs, t):
    ...
    jx = np.minimum(xj, nxpvs - 1.0).astype(int)
...
def run(in_dict, timings):
    ...
    table_dict = {
        **{
            SOIL_VARS_NAMES[k]: numpy_table_to_gt4py_storage(
                SOIL_VARS[k], in_dict["soiltyp"], backend=BACKEND
            )
            for k in range(len(SOIL_VARS))
        },
        **{
            VEG_VARS_NAMES[k]: numpy_table_to_gt4py_storage(
                VEG_VARS[k], in_dict["vegtype"], backend=BACKEND
            )
            for k in range(len(VEG_VARS))
        },
        **{
            "slope": numpy_table_to_gt4py_storage(
                slope_data, in_dict["slopetyp"], backend=BACKEND
            )
        },
    }
    ...
    in_dict = {
        **{k: numpy_to_gt4py_storage(in_dict[k], backend=BACKEND) for k in IN_VARS},
    ...
    out_dict = {
        k: gt4py_to_numpy_storage(out_dict[k], backend=BACKEND) for k in INOUT_VARS
    }
