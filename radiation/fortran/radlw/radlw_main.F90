...
! --------------------------------
      subroutine lwrad                                                  &
     &     ( ser_count, plyr,plvl,tlyr,tlvl,qlyr,olyr,gasvmr,                      &   !  ---  inputs
     &       clouds,icseed,aerosols,sfemis,sfgtmp,                      &
     &       dzlyr,delpin,de_lgth,                                      &
     &       npts, nlay, nlp1, lprnt,                                   &
...
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    ...
    call fs_write_field(ppser_serializer, ppser_savepoint, 'fluxfac', fluxfac)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'heatfac', heatfac)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'exp_tbl', exp_tbl)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'semiss0', semiss0)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'fluxfac', fluxfac)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'heatfac', heatfac)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'exp_tbl', exp_tbl)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'semiss0', semiss0, ppser_zrperturb)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'fluxfac', fluxfac, ppser_zrperturb)
