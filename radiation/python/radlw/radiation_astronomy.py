...
class AstronomyClass():
    ...
    def iw3jdn(self, iyear, month, iday):
        ...
        iw3jdn = iday - 32075 + self.integer_divide_towards_zero(1461 * (iyear + 4800 + self.integer_divide_towards_zero((month - 14), 12)), 4) + \
            self.integer_divide_towards_zero(367 * (month - 2 - self.integer_divide_towards_zero((month -14), 12) * 12), 12) - \
            self.integer_divide_towards_zero(3 * (self.integer_divide_towards_zero((iyear + 4900 + self.integer_divide_towards_zero((month - 14), 12)), 100)), 4)
    ...
    def coszmn(self, xlon, sinlat, coslat, solhr, IM, me):
        ...
        for it in range(self.nstp):
            ...
            for i in range(IM):
                coszn     = self.sindec * sinlat[i] + self.cosdec * coslat[i] * \
                    np.cos(cns+xlon[i])
