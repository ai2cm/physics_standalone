...
from gt4py.gtscript import FORWARD, PARALLEL, Field, computation, interval, stencil, floor, __externals__
...
for var in locvars:
    if var in bandvars:
        ...
        locdict_gt4py[var] = create_storage_zeros(backend, shape, type1)
...
def cldprop(tauliq: Field[type_nbands],
            cldliq: FIELD_FLT):
...
cldprop(locdict_gt4py['tauliq'],
        locdict_gt4py['cldliq'])

#@gtscript.stencil(backend=backend, rebuild=rebuild)
#def cldprop(cfrac: FIELD_FLT, 
#            cliqp: FIELD_FLT, 
#            reliq: FIELD_FLT, 
#            cicep: FIELD_FLT, 
#            reice: FIELD_FLT, 
...
#                        else:
#                            indx = 1
#                        indx = max(1, min(57, floor(min(factor))))
#                        fint = factor - indx
#
#                        for i in range(nbands):
#                            tauliq[0, 0, 0][i] = max(0.0, cldliq*(absliq1[0, 0, 0][indx, i]
#                                + fint*(absliq1[0, 0, 0][indx+1, i]-absliq1[0, 0, 0][indx, i])))
#
#                if cldice <= 0.0:
#                    for i in range(nbands):
#                        tauice[0, 0, 0][i] = 0.0
...
#                        indx = max(1, min(45, floor(factor)))
#                        fint = factor - indx
#
#                        for i in range(nbands):
#                            tauice[0, 0, 0][i] = max(0., cldice*(absice3[0, 0, 0][indx, i] + fint*(absice3[0, 0, 0][indx+1, i] - absice3[0, 0, 0][indx, i])))
#
#                for i in range(nbands):
...
#cldprop(cfrac, cliqp, reliq, cicep, reice, cdat1, cdat3, cdat4,
#        ipseed, dz, de_lgth,
