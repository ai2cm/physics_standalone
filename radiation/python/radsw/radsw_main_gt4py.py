...
class RadSWClass:
    ...
    def __init__(self, me, iovrsw, isubcsw, icldflg):
        ...
        self.ngs = np.tile(np.array(ngs)[None, None, None, :], (npts, 1, nlp1, 1))

        self.NGB = create_storage_from_array(self.NGB, backend, shape_nlp1, type_ngptsw)
        self.ngs = create_storage_from_array(self.ngs, backend, shape_nlp1, type_ngptsw)
        ...
        self.nspa - np.tile(
            np.array(self.nspa)[None, None, None, :], (npts, 1, nlp1, 1)
        )
        self.nspb - np.tile(
            np.array(self.nspb)[None, None, None, :], (npts, 1, nlp1, 1)
        )
        ...
        self.nspa = create_storage_from_array(
            self.nspa, backend, shape_nlp1, (DTYPE_FLT, (nbandssw,))
        )

        self.nspb = create_storage_from_array(
            self.nspb, backend, shape_nlp1, (DTYPE_FLT, (nbandssw,))
        )
    ...
    def _load_lookup_table_data(self):
        ...
        lookupdict_gt4py = loadlookupdata("cldprtb")
