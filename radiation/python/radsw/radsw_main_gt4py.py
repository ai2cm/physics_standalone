...
class RadSWClass:
    ...
    def __init__(self, me, iovrsw, isubcsw, icldflg):
        ...
        outdict_gt4py = create_gt4py_dict_zeros(self.OUTVARS)
        locdict_gt4py = create_gt4py_dict_zeros(self.LOCVARS)
        self.outdict_gt4py = outdict_gt4py
        self.locdict_gt4py = locdict_gt4py

    def set_locdict_zero(self):
        """
        "Zeros" the local GT4Py storages used in the short wave scheme
        """
        for var in self.LOCVARS.keys():
            if len(self.locdict_gt4py[var].shape) == 1:
                self.locdict_gt4py[var][:] = 0.0
            elif len(self.locdict_gt4py[var].shape) == 2:
                self.locdict_gt4py[var][:,:] = 0.0
