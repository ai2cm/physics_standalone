...
def gfdl_cloud_microphys_driver_merged( input_data, hydrostatic, phys_hydrostatic, 
                                        kks, ktop, timings, n_iter, not_first_rep ):
    ...
    for n in range(ntimes):
        ...
        main_loop( h_var, rh_adj, rh_rain, 
                   graupel, ice, rain, snow, 
                   qaz, qgz, qiz, qlz, qrz, qsz, qvz, tz, w, 
                   t0, den0, dz0, 
                   dp1, p1, m1, 
                   ccn, c_praut, 
                   m2_rain, m2_sol, 
                   DTYPE_INT(do_sedi_w), DTYPE_INT(p_nonhydro), DTYPE_INT(use_ccn), 
                   c_air, c_vap, d0_vap, lv00, fac_rc, csacr, cracs, cgacr, cgacs, 
                   acco[0, 0], acco[0, 1], acco[0, 2], acco[0, 3], 
                   acco[1, 0], acco[1, 1], acco[1, 2], acco[1, 3], 
                   acco[2, 0], acco[2, 1], acco[2, 2], acco[2, 3], 
                   csacw, csaci, cgacw, cgaci, cracw, 
                   ...
                   csmlt[0], csmlt[1], csmlt[2], csmlt[3], csmlt[4], 
                   cgmlt[0], cgmlt[1], cgmlt[2], cgmlt[3], cgmlt[4], 
                   ces0, log_10, tice0, t_wfr, 
                   so3, dt_rain, zs, dts, rdts, 
                   fac_i2s, fac_g2v, fac_v2g, fac_imlt, fac_l2v, 
                   exec_info=exec_info )
        
        if BENCHMARK and not_first_rep:
            timings["main_loop_call"][n_iter] += (exec_info['call_end_time'] - exec_info['call_start_time'])
            timings["main_loop_run"][n_iter]  += (exec_info['run_end_time'] - exec_info['run_start_time'])
