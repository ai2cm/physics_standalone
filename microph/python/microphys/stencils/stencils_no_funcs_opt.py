    ...
    with computation(PARALLEL), interval(...):
        ...
        if no_fall == 0:
            ...
            if (tz > t_wfr) and (qrz > qrmin):
                ...
                # Define heat capacity and latent heat coefficient
                lhl_rr1   = lv00 + d0_vap * tz
                q_liq_rr1 = qlz + qrz
                q_sol_rr1 = qiz + qsz + qgz
                cvm_rr1   = c_air + qvz * c_vap + q_liq_rr1 * c_liq + q_sol_rr1 * c_ice
                lcpk_rr1  = lhl_rr1 / cvm_rr1

                tin_rr1 = tz - lcpk_rr1 * qlz    # Presence of clouds suppresses the rain evap
                qpz_rr1 = qvz + qlz
                
                qsat_rr1  = ( e00 * exp((dc_vap * log(tin_rr1 / t_ice) + lv0 * (tin_rr1 - t_ice) / (tin_rr1 * t_ice)) / rvgas) ) / \
                            ( rvgas * tin_rr1 * den )
                dqsdt_rr1 = qsat_rr1 * (dc_vap + lv0 / tin_rr1) / (rvgas * tin_rr1)
                
                dqh_rr1     = max(qlz, h_var * max(qpz_rr1, qcmin))
                dqh_rr1     = min(dqh_rr1, 0.2 * qpz_rr1)   # New limiter
                dqv_rr1     = qsat_rr1 - qvz                # Use this to prevent super-sat the gird box
                q_minus_rr1 = qpz_rr1 - dqh_rr1
                q_plus_rr1  = qpz_rr1 + dqh_rr1

                # qsat must be > q_minus to activate evaporation
                # qsat must be < q_plus to activate accretion
                # Rain evaporation
                if (dqv_rr1 > qvmin) and (qsat_rr1 > q_minus_rr1):
                
                    if qsat_rr1 > q_plus_rr1:
                        
                        dq_rr1 = qsat_rr1 - qpz_rr1
                        
                    else:
                        
                        # q_minus < qsat < q_plus
                        # dq == dqh if qsat == q_minus
                        dq_rr1 = 0.25 * (q_minus_rr1 - qsat_rr1)**2 / dqh_rr1
                    
                    qden_rr1 = qrz * den
                    t2_rr1   = tin_rr1 * tin_rr1
                    evap_rr1 = crevp_0 * t2_rr1 * dq_rr1 * \
                               ( crevp_1 * sqrt(qden_rr1) + crevp_2 * \
                                 exp(0.725 * log(qden_rr1)) ) / \
                               ( crevp_3 * t2_rr1 + crevp_4 * qsat_rr1 * den )
                    evap_rr1 = min(qrz, min(dt5 * evap_rr1, dqv_rr1 / (1. + lcpk_rr1 * dqsdt_rr1)))

                    # Alternative minimum evap in dry environmental air
                    qrz       = qrz - evap_rr1
                    qvz       = qvz + evap_rr1
                    q_liq_rr1 = q_liq_rr1 - evap_rr1
                    cvm_rr1   = c_air + qvz * c_vap + q_liq_rr1 * c_liq + q_sol_rr1 * c_ice
                    tz        = tz - evap_rr1 * lhl_rr1 / cvm_rr1
                    
                # Accretion: pracc
                if (qrz > qrmin) and (qlz > 1.e-6) and (qsat_rr1 < q_minus_rr1):
                    
                    sink_rr1 = dt5 * denfac * cracw * exp(0.95 * log(qrz * den))
                    sink_rr1 = sink_rr1 / (1. + sink_rr1) * qlz
                    qlz      = qlz - sink_rr1
                    qrz      = qrz + sink_rr1
    ...
    with computation(PARALLEL), interval(...):
        ...
        if no_fall == 0:
            ...
            if (tz > t_wfr) and (qrz > qrmin):
                ...
                # Define heat capacity and latent heat coefficient
                lhl_rr2   = lv00 + d0_vap * tz
                q_liq_rr2 = qlz + qrz
                q_sol_rr2 = qiz + qsz + qgz
                cvm_rr2   = c_air + qvz * c_vap + q_liq_rr2 * c_liq + q_sol_rr2 * c_ice
                lcpk_rr2  = lhl_rr2 / cvm_rr2

                tin_rr2 = tz - lcpk_rr2 * qlz    # Presence of clouds suppresses the rain evap
                qpz_rr2 = qvz + qlz
                
                qsat_rr2  = ( e00 * exp((dc_vap * log(tin_rr2 / t_ice) + lv0 * (tin_rr2 - t_ice) / (tin_rr2 * t_ice)) / rvgas) ) / \
                            ( rvgas * tin_rr2 * den )
                dqsdt_rr2 = qsat_rr2 * (dc_vap + lv0 / tin_rr2) / (rvgas * tin_rr2)
                
                dqh_rr2     = max(qlz, h_var * max(qpz_rr2, qcmin))
                dqh_rr2     = min(dqh_rr2, 0.2 * qpz_rr2)   # New limiter
                dqv_rr2     = qsat_rr2 - qvz                # Use this to prevent super-sat the gird box
                q_minus_rr2 = qpz_rr2 - dqh_rr2
                q_plus_rr2  = qpz_rr2 + dqh_rr2

                # qsat must be > q_minus to activate evaporation
                # qsat must be < q_plus to activate accretion
                # Rain evaporation
                if (dqv_rr2 > qvmin) and (qsat_rr2 > q_minus_rr2):
                
                    if qsat_rr2 > q_plus_rr2:
                        
                        dq_rr2 = qsat_rr2 - qpz_rr2
                        
                    else:
                        
                        # q_minus < qsat < q_plus
                        # dq == dqh if qsat == q_minus
                        dq_rr2 = 0.25 * (q_minus_rr2 - qsat_rr2)**2 / dqh_rr2
                    
                    qden_rr2 = qrz * den
                    t2_rr2   = tin_rr2 * tin_rr2
                    evap_rr2 = crevp_0 * t2_rr2 * dq_rr2 * \
                               ( crevp_1 * sqrt(qden_rr2) + crevp_2 * \
                                 exp(0.725 * log(qden_rr2)) ) / \
                               ( crevp_3 * t2_rr2 + crevp_4 * qsat_rr2 * den )
                    evap_rr2 = min(qrz, min(dt5 * evap_rr2, dqv_rr2 / (1. + lcpk_rr2 * dqsdt_rr2)))

                    # Alternative minimum evap in dry environmental air
                    qrz       = qrz - evap_rr2
                    qvz       = qvz + evap_rr2
                    q_liq_rr2 = q_liq_rr2 - evap_rr2
                    cvm_rr2   = c_air + qvz * c_vap + q_liq_rr2 * c_liq + q_sol_rr2 * c_ice
                    tz        = tz - evap_rr2 * lhl_rr2 / cvm_rr2
                    
                # Accretion: pracc
                if (qrz > qrmin) and (qlz > 1.e-6) and (qsat_rr2 < q_minus_rr2):
                    
                    sink_rr2 = dt5 * denfac * cracw * exp(0.95 * log(qrz * den))
                    sink_rr2 = sink_rr2 / (1. + sink_rr2) * qlz
                    qlz      = qlz - sink_rr2
                    qrz      = qrz + sink_rr2
        ...
        if irain_f != 0:
            ...
            if tz > t_wfr:
                ...
                if dq > 0.:
                    ...
                    sink_nsv = min(dq, dt_rain * c_praut * den * exp(so3 * log(qlz)))
                    qlz      = qlz - sink_nsv
                    qrz      = qrz + sink_nsv
        ...
        if irain_f == 0:
            ...
            if tz > t_wfr + dt_fr:
                ...
                if dq > 0.:     # q_plus > qc
                    ...
                    sink_sv = min(1., dq / dl) * dt_rain * c_praut * den * exp(so3 * log(qlz))
                    qlz     = qlz - sink_sv
                    qrz     = qrz + sink_sv
    ...
    with computation(PARALLEL), interval(...):
        ...
        if const_vi == 1:
            ...
            if qiz < thi:
                ...
                tc_fs = tz - tice
                vtiz  = (3. + log(qiz * den) / log_10) * (tc_fs * (aa * tc_fs + bb) + cc) + dd_fs * tc_fs + ee
...
def icloud( h_var   : FIELD_FLT, 
    ...
    with computation(PARALLEL), interval(...):
            ...
            if tc >= 0.:
                ...
                if qsz > qcmin:
                    ...
                    if qlz > qrmin:
                        ...
                        factor_ic = denfac * csacw * exp(0.8125 * log(qsz * den))
                        psacw     = factor_ic / (1. + dts * factor_ic) * qlz # Rate
                    ...
                    sink_ic = min(qsz, min(dts * (psmlt + pracs), tc / icpk_ic))
                    qsz     = qsz - sink_ic
                    tmp_ic  = min(sink_ic, dim(qs_mlt, qlz)) # Maximum ql due to snow melt
                    qlz     = qlz + tmp_ic
                    qrz     = qrz + sink_ic - tmp_ic
                    q_liq   = q_liq + sink_ic
                    q_sol   = q_sol - sink_ic
                    cvm     = c_air + qvz * c_vap + q_liq * c_liq + q_sol * c_ice
                    tz      = tz - sink_ic * lhi_ic / cvm
                    tc      = tz - tice
                ...
                if (qgz > qcmin) and (tc > 0.):
                    ...
                    if qlz > qrmin:
                        ...
                        factor_ic = cgacw * qden / sqrt(den * sqrt(sqrt(qden)))
                        pgacw     = factor_ic / (1. + dts * factor_ic) * qlz # Rate
                
                    # pgmlt: graupel melt
                    pgmlt = dts * ( ( cgmlt_0 * tc / den - cgmlt_1 * dqs0 ) *                       \
                                    ( cgmlt_2 * sqrt(qden) + cgmlt_3 * qden**0.6875 / den**0.25 ) + \
                                    cgmlt_4 * tc * (pgacw + pgacr) )
                    pgmlt = min(max(0., pgmlt), min(qgz, tc / icpk_ic))
                    qgz   = qgz - pgmlt
                    qrz   = qrz + pgmlt
                    q_liq = q_liq + pgmlt
                    q_sol = q_sol - pgmlt
                    cvm   = c_air + qvz * c_vap + q_liq * c_liq + q_sol * c_ice
                    tz    = tz - pgmlt * lhi_ic / cvm
                ...
                if qiz > 3.e-7: # Cloud ice sink terms
                    ...
                    if qsz > 1.e-7:
                        ...
                        factor_ic = dts * denfac * csaci * exp(0.05 * tc + 0.8125 * log(qsz * den))
                        psaci     = factor_ic / (1. + factor_ic) * qiz
                    ...
                    sink_ic = min(0.75 * qiz, psaci + psaut)
                    qiz     = qiz - sink_ic
                    qsz     = qsz + sink_ic
                    ...
                    if qgz > 1.e-6:
                        ...
                        factor_ic = dts * cgaci * sqrt(den) * qgz
                        pgaci     = factor_ic / (1. + factor_ic) * qiz
                        qiz       = qiz - pgaci
                        qgz       = qgz + pgaci
                ...
                if (qrz > 1e-7) and (tc < 0.):
                    ...
                    # Total sink to qr
                    sink_ic   = psacr + pgfr
                    factor_ic = min(sink_ic, min(qrz, -tc / icpk_ic)) / max(sink_ic, qrmin)
                    
                    psacr = factor_ic * psacr
                    pgfr  = factor_ic * pgfr
                ...
                if qsz > 1.e-7:
                    ...
                    if qgz > qrmin:
                        ...
                        sink_ic = dts * cgacs * abs(vtgz - vtsz) * qsz * s2 * ( acco_03 * t1 + acco_13 * sqrt(t1) * s2 + acco_23 * s1 )
                    ...
                    if qsz > qsm:
                        ...
                        factor_ic = dts * 1.e-3 * exp(0.09 * (tz - tice))
                        sink_ic   = sink_ic + factor_ic / (1. + factor_ic) * (qsz - qsm)
                    
                    sink_ic = min(qsz, sink_ic)
                    qsz     = qsz - sink_ic
                    qgz     = qgz + sink_ic
                ...
                if (qgz > 1.e-7) and (tz < tice0):
                    ...
                    if qlz > 1.e-6:
                        ...
                        qden   = qgz * den
                        factor_ic = dts * cgacw * qden / sqrt(den * sqrt(sqrt(qden)))
                        pgacw     = factor_ic / (1. + factor_ic) * qlz
                    ...
                    sink_ic   = pgacr + pgacw
                    factor_ic = min(sink_ic, dim(tice, tz) / icpk_ic) / max(sink_ic, qrmin)
                    pgacr     = factor_ic * pgacr
                    pgacw     = factor_ic * pgacw
        ...
        icpk_ic = lhi_ic / cvm
        tcpk    = lcpk + icpk_ic
        tcp3    = lcpk + icpk_ic * min(1., dim(tice, tz) / (tice - t_wfr))
        ...
        if p1 >= p_min:
            ...
            if tz < t_min:
                ...
                sink_ic = dim(1.e-7, qvz)
                qvz     = qvz - sink_ic
                qiz     = qiz + sink_ic
                q_sol   = q_sol + sink_ic
                cvm     = c_air + qvz * c_vap + q_liq * c_liq + q_sol * c_ice
                tz      = tz + sink_ic * (lhl + lhi_ic) / cvm
                ...
                if ((tin > t_sub + 6.) and (rh >= rh_adj)) or (tin <= t_sub + 6.):
                    ...
                    if dq0 > 0.:
                        ...
                        factor_ic = min(1., fac_l2v * (10. * dq0 / qsw))   # The rh dependent factor = 1 at 90%
                        evap      = min(qlz, factor_ic * dq0 / (1. + tcp3 * dwsdt))
                    ...
                    dtmp_ic = t_wfr - tz   # [-40, -48]
                    
                    if (dtmp_ic > 0.) and (qlz > qcmin):
                    
                        sink_ic = min(qlz, min(qlz * dtmp_ic * 0.125, dtmp_ic / icpk_ic))
                        qlz     = qlz - sink_ic
                        qiz     = qiz + sink_ic
                        q_liq   = q_liq - sink_ic
                        q_sol   = q_sol + sink_ic
                    ...
                    if fast_sat_adj == 1:
                        ...
                        if (qlz > qrmin) and (tc > 0.):
                            ...
                            sink_ic = 3.3333e-10 * dts * (exp(0.66 * tc) - 1.) * den * qlz * qlz
                            sink_ic = min(qlz, min(tc / icpk_ic, sink_ic))
                            qlz     = qlz - sink_ic
                            qiz     = qiz + sink_ic
                            q_liq   = q_liq - sink_ic
                            q_sol   = q_sol + sink_ic
                    ...
                    if tz < tice:
                        ...
                        if dq > 0.: # Vapor -- > ice
                            ...
                            qi_crt_ic = qi_gen * min(qi_lim, 0.1 * tmp_ic) / den
                            sink_ic   = min(sink_ic, min(max(qi_crt_ic - qiz, pidep), tmp_ic / tcpk))
                    
                        else:   # Ice -- > vapor
                    
                            pidep   = pidep * min(1., dim(tz, t_sub) * 0.2)
                            sink_ic = max(pidep, max(sink_ic, -qiz))
                    ...
                    if qsz > qrmin:
                        ...
                        qden   = qsz * den
                        tmp_ic = exp(0.65625 * log(qden))
                        tsq    = tz * tz
                        dq     = (qsi - qvz) / (1. + tcpk * dqsdt)
                        pssub  = cssub_0 * tsq *                                              \
                                 ( cssub_1 * sqrt(qden) + cssub_2 * tmp_ic * sqrt(denfac) ) / \
                    ...
                    if qrz > qcmin:
                        ...
                        sink_ic = min(qrz, dim(rh_rain * qsw, qvz) / (1. + lcpk * dqsdt))
                        qvz     = qvz + sink_ic
                        qrz     = qrz - sink_ic
                        q_liq   = q_liq - sink_ic
                        cvm     = c_air + qvz * c_vap + q_liq * c_liq + q_sol * c_ice
                        tz      = tz - sink_ic * lhl / cvm
                    '''
                    END USE_MIN_EVAP
                    '''
                    ...
                    if do_qa == 0:
                        ...
                        tin = tz - (lcpk * q_cond + icpk_ic * q_sol)   # Minimum temperature
                        ...
                        if tin <= t_wfr:
                            ...
                            if q_cond > 3.e-6:
                                ...
                                '''
                                rqi = (tice - tin) / (tice - t_wfr_tmp_ic)
