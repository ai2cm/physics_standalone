...
for tile in range(num_tiles):
    for ser_count in range(ser_count_max):
        ...
        for n in range(REPS+1):
            ...
            # Start driver timer
            if n > 0: t_driver_start = tm.perf_counter()
            ...
            refl_10cm \
            = gfdl_cloud_microphys_driver(scaled_data, False, True, 0, 0, timings, 0, (n > 0))
...
output_file = open("./out/timings_benchmark_{}_{}_C{}.dat".format(BACKEND, STENCILS, CN), "w")
for var in timings:
    if var != "tot": timings[var][0] /= REPS
if STENCILS != "merged":                
    timings["main_loop_call"][0] = timings["warm_rain_1_call"][0] + timings["sedimentation_call"][0] + timings["warm_rain_2_call"][0] + timings["icloud_call"][0]
    timings["main_loop_run"][0]  = timings["warm_rain_1_run"][0] + timings["sedimentation_run"][0] + timings["warm_rain_2_run"][0] + timings["icloud_run"][0]
for var in timings:
    if var != "tot": 
