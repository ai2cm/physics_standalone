...
def run(in_dict, in_dict2, backend):
    ...
    prepare_sflx(in_dict["t1"], in_dict["q1"], in_dict["cm"], in_dict["ch"], in_dict["prsl1"],
                 ...
                 in_dict["flag_guess"], in_dict["fpvs"], out_dict["weasd"], out_dict["snwdph"], 
                 out_dict["tskin"], out_dict["tprcp"], out_dict["srflag"], out_dict["canopy"], out_dict["zorl"], 
                 out_dict["smc"], out_dict["stc"], out_dict["slc"], scalar_dict["delt"])
    ...
    out_dict = {**{k: gt4py_to_numpy_storage_onelayer(
        out_dict[k], backend=backend) for k in INOUT_VARS}, **{k: gt4py_to_numpy_storage_onelayer(
...
def prepare_sflx(
    t1: DT_F, q1: DT_F, cm: DT_F, ch: DT_F, prsl1: DT_F, prslki: DT_F, zf: DT_F, land: DT_I, wind: DT_F, flag_iter: DT_I, flag_guess: DT_I,
    fpvs: DT_F, weasd: DT_F, snwdph: DT_F, tskin: DT_F, tprcp: DT_F, srflag: DT_F, 
    canopy: DT_F, zorl: DT_F, smc: DT_F2, stc: DT_F2, slc: DT_F2, delt: float
):
