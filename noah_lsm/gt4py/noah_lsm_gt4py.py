...
OUT_VARS = ["weasd", "snwdph", "tskin", "tprcp", "srflag", "smc0", "smc1", "smc2", "smc3",
...
SCALAR_VARS2 = ["c1xpvs", "c2xpvs"]

SOIL_VARS = [bb, satdk, satdw, f11, satpsi,
             qtz, drysmc, maxsmc, refsmc, wltsmc]
VEG_VARS = [nroot_data, snupx, rsmtbl, rgltbl, hstbl, lai_data]
SOIL_VARS_NAMES = ["bb", "satdk", "satdw", "f11", "satpsi",
                   "qtz", "drysmc", "maxsmc", "refsmc", "wltsmc"]
VEG_VARS_NAMES = ["nroot_data", "snupx",
                  "rsmtbl", "rgltbl", "hstbl", "lai_data"]
...
def run(in_dict, in_dict2, backend):
    ...
    scalar_dict = {**{k: in_dict[k] for k in SCALAR_VARS},
                   **{k: in_dict2[k] for k in SCALAR_VARS2}}
    ...
    in_dict = {**{k: numpy_to_gt4py_storage(
        in_dict[k], backend=backend) for k in IN_VARS}, **{k: numpy_to_gt4py_storage(
            in_dict2[k], backend=backend) for k in IN_VARS2}}
    # compile stencil
    ...
    sfc_drv(**in_dict, **out_dict, **scalar_dict, **table_dict)
...
def nopac_fn(
    # inputs
    nroot, etp, prcp, smcmax, smcwlt, smcref,
    ...
    if etp > 0.:
        eta1, edir1, ec1, et1, ett1 = evapo_fn(nroot, cmc, cmcmax, etp1, dt,
                                               sh2o0, sh2o1, sh2o2, sh2o3, smcmax, smcwlt, smcref, smcdry, pc,
